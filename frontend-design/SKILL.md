...
description: >-
  Creates distinctive, production-grade frontend interfaces with high design quality. Use this skill
...
Commit fully to the chosen aesthetic direction. Distinctive, memorable design comes from creative
ambition and precise execution — not timid defaults.

## Output Format

Match the output format to the context:

- **Single HTML file**: self-contained with `<style>` and `<script>` inline; load fonts via Google
  Fonts CDN. Default for artifacts, posters, and standalone demos.
- **React / Vue component**: follow the framework's conventions; import fonts via a CSS file or
  the framework's font plugin.
- **Multi-file project**: separate HTML/CSS/JS only when the user explicitly requests it.

## Accessibility Baseline

Distinctive design and accessibility are not opposites. Always:

- Meet WCAG AA contrast ratio: 4.5:1 for body text, 3:1 for large text and UI components
- Support keyboard navigation — visible focus states, logical tab order
- Respect `prefers-reduced-motion` — wrap all non-essential animations:

```css
@media (prefers-reduced-motion: no-preference) {
  .animated-element { animation: slide-in 0.4s ease; }
}
```
