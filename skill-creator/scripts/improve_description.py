...
Takes eval results (from run_eval.py) and generates an improved description by calling an LLM CLI
tool as a subprocess. The CLI command defaults to `claude` but can be overridden via the
SKILL_LLM_CLI environment variable.
"""
...
# The CLI tool name (and session env var) are configurable via environment.
