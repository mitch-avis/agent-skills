...
✅ `1.Text` `①Text` `(1)Text` `Step 1:Text` ❌ `1. Text`

### Safe Subgraph Syntax

✅ `subgraph id["Name"]` `subgraph simple_name` ❌ `subgraph Name With Spaces`

### Safe Node References

✅ `NodeID --> AnotherID` ❌ `"Display Text" --> "Other Text"`

### Safe Special Characters

✅ `『』` for quotes, `「」` for parentheses ❌ `"` unescaped quotes, `()` in problematic contexts
