...
curl -L \
  https://github.com/kubernetes-sigs/cluster-api/releases/download/v1.6.0/clusterctl-linux-amd64 \
  -o clusterctl
...
subctl join --kubeconfig kubeconfig-cluster1 \
  broker-info.subm --clusterid cluster1
subctl join --kubeconfig kubeconfig-cluster2 \
...
### ArgoCD Multi-Cluster (Cluster Generator)
...
```yaml
# Primary cluster — weighted DNS via ExternalDNS
            ...
            backend:
              service:
                ...
                port:
                  number: 80
---
# Secondary cluster — weight 0 (standby)
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: myapp
