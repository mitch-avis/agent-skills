...
```yaml
# Sizing guidance:
#   requests: average usage + 10-20% buffer
#   CPU limits: 2-4x requests (burst capacity)
#   memory limits: 1.5-2x requests (OOM prevention)
resources:
  requests:
    cpu: 100m
    memory: 128Mi
...
spec:
  ...
  updatePolicy:
    ...
    # Auto  — apply on creation and during runtime (restarts pod)
    updateMode: "Auto"
  resourcePolicy:
    containerPolicies:
      - containerName: myapp
...
## HPA with Custom Metrics and Scale Behavior
...
spec:
  ...
  metrics:
    ...
    # Custom metric (e.g., from Prometheus adapter)
    - type: Pods
      pods:
        metric:
...
spec:
  template:
    spec:
      ...
      containers:
        - name: processor
          image: myregistry.io/batch:v1.0.0
          resources:
            requests:
              cpu: 500m
              memory: 512Mi
...
spec:
  template:
    spec:
      ...
      containers:
        - name: worker
          image: myregistry.io/worker:v1.0.0
```

## Best Practices

1. **Set resource requests** on all containers for efficient scheduling
2. **Use VPA recommendations** to right-size workloads
3. **Tune HPA stabilization** to prevent scaling thrashing
...
8. **Label resources** for cost attribution (team, environment, cost-center)
9. **Schedule dev environments** to scale down off-hours
10. **Monitor with Kubecost** or cloud-native cost tools
11. **Use priority classes** to ensure critical workloads run first
